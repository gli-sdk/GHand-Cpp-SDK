...
// Pull pose
std::vector<ghand::JointCommand> MakePullPose() {
  return {
...
int main() {
  ...
  while (true) {
    ...
    if (!Pull(*hand)) {
      std::cout << "Round " << gesture_cycle << " pull action execution failed"
                << '\n';
      break;
    }
