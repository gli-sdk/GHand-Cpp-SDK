...
std::vector<JointCommand> MakeClapPose() {
    return {
        {JointId::THUMB_PIP, 15.0f}, {JointId::THUMB_MCP, 0.0f},
        {JointId::THUMB_SWING, 30.0f}, {JointId::THUMB_ROTATION, 0.0f},
        {JointId::FF_PIP, 15.0f}, {JointId::FF_MCP, 19.0f}, {JointId::FF_SWING, 0.0f},
        {JointId::MF_PIP, 0.0f}, {JointId::MF_MCP, 20.0f},
        {JointId::RF_PIP, 19.0f}, {JointId::RF_MCP, 0.0f},
        {JointId::LF_PIP, 14.0f}, {JointId::LF_MCP, 6.0f},
    };
}
...
int main() {
    ...
    while (true) {
        ...
        if (!Clap(*hand)) {
            std::cout << "第 " << gesture_cycle << " 轮演示中的拍手动作执行失败" << std::endl;
            break;
        }
