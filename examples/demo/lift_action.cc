...
std::vector<JointCommand> MakeLiftPose() {
    return {
        ...
        {JointId::FF_PIP, 60.0f}, {JointId::FF_MCP, 35.0f}, {JointId::FF_SWING, 0.0f},
        {JointId::MF_PIP, 60.0f}, {JointId::MF_MCP, 35.0f},
        {JointId::RF_PIP, 60.0f}, {JointId::RF_MCP, 35.0f},
        {JointId::LF_PIP, 60.0f}, {JointId::LF_MCP, 35.0f},
    };
}
...
int main() {
    ...
    while (true) {
        ...
        if (!Lift(*hand)) {
            std::cout << "第 " << gesture_cycle << " 轮演示中的拎起动作执行失败" << std::endl;
            break;
        }
