...
#define INVALID_CHANNEL_HANDLE 0
	 ...
	 long  _stdcall  ZCAN_OpenDevice(int device_type, int device_index, int reserved);

	 long _stdcall ZCAN_CloseDevice(long handle);

	 long _stdcall  ZCAN_GetDeviceInf(long handle, ZCAN_DEVICE_INFO &pInfo);

	 Handle_chl*  _stdcall  ZCAN_InitCAN(long handle, UINT can_index, ZCAN_CHANNEL_INIT_CONFIG &pConfig);

	 IProperty*  _stdcall GetIProperty(long handle);


	 void testCAN(Handle_chl &handle_chl);

	  int _stdcall  ZCAN_StartCAN(Handle_chl &handle_chl);

	 UINT _stdcall ZCAN_IsDeviceOnLine(long device_handle);

	 UINT _stdcall ZCAN_ResetCAN(Handle_chl &handle_chl);

	 UINT _stdcall ZCAN_ClearBuffer(long dllexport_device_handle);

	 UINT _stdcall ZCAN_Transmit(Handle_chl &handle_chl, ZCAN_Transmit_Data* pTransmit, UINT len);

	 UINT _stdcall ZCAN_TransmitFD(Handle_chl &handle_chl, ZCAN_TransmitFD_Data* pTransmit, UINT len);


	 UINT _stdcall ZCAN_SetValue(long device_handle, char* path, char* value);

	 char* _stdcall ZCAN_GetValue(long device_handle, char* path);



	 UINT _stdcall ZCAN_ReadChannelErrInfo(Handle_chl &handle_chl, ZCAN_CHANNEL_ERROR_INFO &errInfo);
	/*
	通道句柄值。
	type
	获取CAN或CANFD报文，0 = CAN，1 = CANFD。*/
	 UINT _stdcall ZCAN_GetReceiveNum(Handle_chl &handle_chl, BYTE type);
	//wait_time 默认值 DEF(-1)
	 UINT _stdcall ZCAN_Receive(Handle_chl &handle_chl, ZCAN_Receive_Data  *pReceive, UINT len, int wait_time);
	//wait_time 默认值 DEF(-1)
	 UINT _stdcall  ZCAN_ReceiveFD(Handle_chl &handle_chl, ZCAN_ReceiveFD_Data *pReceive, UINT len, int wait_time);

	 UINT _stdcall  ZCAN_SetAbitBaud(Handle_chl &handle_chl, UINT can_index, UINT dbitbaud);

	 UINT _stdcall  ZCAN_SetDbitBaud(Handle_chl &handle_chl, UINT can_index, UINT dbitbaud);

	 UINT _stdcall  ZCAN_SetBaudRateCustom(Handle_chl &handle_chl, UINT can_index, char * RateCustom);

	 UINT _stdcall  ZCAN_SetCANFDStandard(Handle_chl &handle_chl, UINT can_index, UINT canfd_standard);

	 UINT _stdcall  ZCAN_SetResistanceEnable(Handle_chl &handle_chl, UINT can_index, UINT enable);

	 UINT _stdcall  ZCAN_ClearFilter(Handle_chl &handle_chl);//清除过滤器

	 UINT _stdcall  ZCAN_SetFilterMode(Handle_chl &handle_chl, UINT mode);//设置过滤moshi

	 UINT _stdcall  ZCAN_SetFilterStartID(Handle_chl &handle_chl, UINT startID);

	 UINT _stdcall  ZCAN_SetFilterEndID(Handle_chl &handle_chl, UINT endID);

	 UINT  _stdcall  ZCAN_AckFilter(Handle_chl &handle_chl);
