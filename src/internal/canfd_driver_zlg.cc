 ...
 *  DeviceType 为 zlgcan.h 中宏定义对应的数字（如 "42" 对应 ZCAN_USBCANFD_100U）。
 *  例如："42:0:0"
 */
static std::string FrameTypeString(canfd::FrameType ft) {
    switch (ft) {
        case canfd::BROADCAST:     return "BC";
        case canfd::COMMAND:       return "CMD";
        case canfd::RESPONSE:      return "RESP";
        case canfd::ACTIVE_REPORT: return "ARPT";
        default:                   return "?";
    }
}
...
static std::vector<ComPortEntry> EnumerateComPorts() {
    ...
    HDEVINFO hDevInfo = SetupDiGetClassDevs(
        &GUID_DEVINTERFACE_COMPORT, NULL, NULL,
    ...
    for (DWORD i = 0; SetupDiEnumDeviceInfo(hDevInfo, i, &devInfoData); ++i) {
        ...
        if (!SetupDiGetDeviceRegistryPropertyA(hDevInfo, &devInfoData,
                SPDRP_HARDWAREID, &dataType, (PBYTE)hwId, sizeof(hwId), &reqSize)) {
            continue;
        }
        ...
        SetupDiGetDeviceRegistryPropertyA(hDevInfo, &devInfoData,
            SPDRP_FRIENDLYNAME, &dataType, (PBYTE)friendly, sizeof(friendly), &reqSize);

        char device_desc[256] = {};
        reqSize = 0;
        SetupDiGetDeviceRegistryPropertyA(hDevInfo, &devInfoData,
            SPDRP_DEVICEDESC, &dataType, (PBYTE)device_desc, sizeof(device_desc), &reqSize);
...
static bool IsZLGVID(int vid) {
    return vid == 0x3562 || vid == 0x0483;
}
...
static bool ResolveComPortToZLGDevice(const std::string& port_name,
    ...
    std::transform(target_port.begin(), target_port.end(), target_port.begin(), ::toupper);
    ...
    if (vidpid_type != 0 && try_open(vidpid_type)) {
        LOG_INFO("Resolved " << port_name << " -> ZLG device type=" << *out_device_type
                 << " index=" << *out_device_index << " (from VID/PID)");
        return true;
    }
...
public:
    ...
    int Open(const std::string& name, uint32_t bitrate, uint32_t dbitrate) override {
        Close();

        // 解析 name: "DeviceType:DeviceIndex:CanIndex[:AbitSamplePoint:DbitSamplePoint]"
        // 或 "COM3" 等 COM 端口名（Windows 下自动解析为 ZLG 设备参数）
        ...
        if (parts.size() >= 1) {
            ...
            if (it != type_map.end()) {
                ...
                try {
                    ...
                    LOG_ERROR("Invalid ZLG device type: " << parts[0]
                              << ". Please use the macro name defined in zlgcan.h (e.g., \"ZCAN_USBCANFD_100U:0:0\") "
        ...
        if (parts.size() >= 2) {
            try { device_index = std::stoi(parts[1]); } catch (...) {}
        }
        if (parts.size() >= 3) {
            try { can_index = std::stoi(parts[2]); } catch (...) {}
        }
        if (parts.size() >= 4) {
            try { abit_sample_point = std::stoi(parts[3]); } catch (...) {}
        }
        if (parts.size() >= 5) {
            try { dbit_sample_point = std::stoi(parts[4]); } catch (...) {}
        }
        ...
        if (device_handle_ == INVALID_DEVICE_HANDLE) {
            LOG_ERROR("ZCAN_OpenDevice failed: type=" << device_type << " index=" << device_index
                                                      << " err=" << GetLastError());
            return -1;
        }
        ...
        if (prop) {
            ...
            if (abit_sample_point > 0) {
                ...
                int c=prop->SetValue(abit_sp_path.c_str(), std::to_string(abit_sample_point).c_str());
            }
            if (dbit_sample_point > 0) {
                std::string dbit_sp_path = ch_str + "/canfd_dbit_sample_point";
                int d=prop->SetValue(dbit_sp_path.c_str(), std::to_string(dbit_sample_point).c_str());
            }

            //4.设置终端电阻（不阻塞启动，失败后仅记录日志）
            std::string res_path = ch_str + "/initenal_resistance";
            if (1 != prop->SetValue(res_path.c_str(), "0")) {
                LOG_WARNING("Failed to set initenal_resistance for channel " << can_index
                         << ", try internal_resistance");
                res_path = ch_str + "/internal_resistance";
                if (1 != prop->SetValue(res_path.c_str(), "0")) {
                    LOG_WARNING("Failed to set internal_resistance for channel " << can_index);
                }
            }
        }
        ...
        channel_handle_ = ZCAN_InitCAN(device_handle_, static_cast<UINT>(can_index), config);
        ...
        LOG_INFO("ZLG CANFD opened: type=" << device_type
                 << " index=" << device_index
                 << " channel=" << can_index
                 << " bitrate=" << bitrate
                 << " dbitrate=" << dbitrate);
    ...
    bool IsOpen() const override {
        return device_handle_ != INVALID_DEVICE_HANDLE && channel_handle_ != nullptr;
    }
